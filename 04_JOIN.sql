...
-- 부서명이 '해외영업2부'인 사원의
-- 사번(EMP_ID), 이름(EMP_NAEM), 부서명(DEPT_TITLE)을 
-- 사번 오름차순으로 조회
...
--  모든 행이 최종 결과(RESULT SET)에 포홤 되도록 하는 JOIN
-- == LEFT JOIN
...
-- JOB.JOB_CODE  		 (CHAR(2))
	--> 컬럼명, 자료형이 모두 일치! == NATUAL JOIN의 대상
...
/* NATUAL JOIN 실패 --> CROSS JOIN 결과 조회*/
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE 
...
SELECT EMP_ID, EMP_NAME , DEPT_TITLE
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID) -- NATURAL JOIN JOB
