...
-- DEPARTMENT 부분에 NULL값이 없어서 연결을 하지 못함 그래서 최종 결과에 포함되지 않는다.
-- 이러한 상태를 INNER JOIN 이라고 한다.
...
FROM EMPLOYEE
NATURAL JOIN JOB -- 자연조인(컬럼명, 타입이 일치하는 컬럼끼리 연결)
...
WHERE SALARY > (
				...
				-- MAIN한테 전달 받은 JOB_CODE의 직급의 평균 급여 조회
);

/* 1) 메인 쿼리 한 행의 값을 서브 쿼리로 전달
 * 2) 서브 쿼레이서 전달 받은 값을 이용해서 SELECT 수행
...
-- ROWNUW 순서가 급여 순서가 아닌, 조화된 행 순서로 지정됨
