...
-- ORA-00001: 무결성 제약 조건(KH_SIJ.USER_ID_U)에 위배됩니다
...
-- 참조(REFERENCES)된 다른 테이블의 컬럼이 제공하는 값만 사용할 수 있음
 ...
 --  PK 또는 U 제약 조건이 설정되어 있어야 한다!

-- FOREIGN KEY제약조건에 의해서 테이블간의 관계(RELATIONSHIP)가 형성됨 -- 관계형성
...
-- ORA-02291: 무결성 제약조건(KH_SIJ.GRADE_CODE_FK1)이 위배되었습니다
-- - 부모 키가 없습니다
 --> 참조하는 테이블(USER-GRADE)에 값이 존재하지 않음
...
SELECT * FROM USER_GRADE2;
SELECT * FROM USER_USED_FK2; -- 10을 참조하던 컬럼이 NULL 로 변함
...
-- ORA-02290: 체크 제약조건(KH_SIJ.GENDER_CHECK)이 위배되었습니다
...
--> NOT NULL 제약조건을 제와한 다른 제약 조건은 복사 X
